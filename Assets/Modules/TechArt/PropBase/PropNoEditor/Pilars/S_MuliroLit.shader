Shader "Custom/teste"
{
    Properties
    {
        ...
        _LayerRTiling("Tiling", Range(0.1, 10)) = 1
        [Space(5)]
        ...
        _LayerGTiling("Tiling", Range(0.1, 10)) = 1
        [Space(5)]
        ...
        _LayerBTiling("Tiling", Range(0.1, 10)) = 1
        [Space(5)]
        ...
        _LayerATiling("Tiling", Range(0.1, 10)) = 1
        [Space(10)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Universal Pipeline keywords - MULTILIGHT COMPLETO
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                half  specular      = _BaseSpecular;
                    ...
                    #ifdef USE_LAYER_R
                        float2 uvR = frac(uv * _LayerRTiling);
                        half3 cR = SAMPLE_TEXTURE2D(_LayerR_Map, sampler_BaseMap, uvR).rgb * _LayerRColor.rgb;
                        half3 nR = UnpackNormalScale(SAMPLE_TEXTURE2D(_LayerR_NormalMap, sampler_BaseMap, uvR), _NormalStrength);
                        half4 pR = SAMPLE_TEXTURE2D(_LayerR_PhysicsMap, sampler_BaseMap, uvR);
                        finalColor    += wR * cR;
                        blendedNormal += wR * nR;
                        roughness     += wR * pR.r;
                        metallic      += wR * pR.g;
                    ...
                    #endif
                    #ifdef USE_LAYER_G
                        float2 uvG = frac(uv * _LayerGTiling);
                        half3 cG = SAMPLE_TEXTURE2D(_LayerG_Map, sampler_BaseMap, uvG).rgb * _LayerGColor.rgb;
                        half3 nG = UnpackNormalScale(SAMPLE_TEXTURE2D(_LayerG_NormalMap, sampler_BaseMap, uvG), _NormalStrength);
                        half4 pG = SAMPLE_TEXTURE2D(_LayerG_PhysicsMap, sampler_BaseMap, uvG);
                        finalColor    += wG * cG;
                        blendedNormal += wG * nG;
                        roughness     += wG * pG.r;
                        metallic      += wG * pG.g;
                    ...
                    #endif
                    #ifdef USE_LAYER_B
                        float2 uvB = frac(uv * _LayerBTiling);
                        half3 cB = SAMPLE_TEXTURE2D(_LayerB_Map, sampler_BaseMap, uvB).rgb * _LayerBColor.rgb;
                        half3 nB = UnpackNormalScale(SAMPLE_TEXTURE2D(_LayerB_NormalMap, sampler_BaseMap, uvB), _NormalStrength);
                        half4 pB = SAMPLE_TEXTURE2D(_LayerB_PhysicsMap, sampler_BaseMap, uvB);
                        finalColor    += wB * cB;
                        blendedNormal += wB * nB;
                        roughness     += wB * pB.r;
                        metallic      += wB * pB.g;
                    ...
                    #endif
                    #ifdef USE_LAYER_A
                        float2 uvA = frac(uv * _LayerATiling);
                        half3 cA = SAMPLE_TEXTURE2D(_LayerA_Map, sampler_BaseMap, uvA).rgb * _LayerAColor.rgb;
                        half3 nA = UnpackNormalScale(SAMPLE_TEXTURE2D(_LayerA_NormalMap, sampler_BaseMap, uvA), _NormalStrength);
                        half4 pA = SAMPLE_TEXTURE2D(_LayerA_PhysicsMap, sampler_BaseMap, uvA);
                        finalColor    += wA * cA;
                        blendedNormal += wA * nA;
                        roughness     += wA * pA.r;
                        metallic      += wA * pA.g;
                ...
                // Cálculo correto da matriz TBN
                float3 normalWS = normalize(IN.normalWS);
                float3 tangentWS = normalize(IN.tangentWS.xyz);
                float3 bitangentWS = cross(normalWS, tangentWS) * IN.tangentWS.w * GetOddNegativeScale();
                
                // Aplicar normal map usando a matriz TBN correta
                float3 finalNormalWS = TransformTangentToWorld(blendedNormal, half3x3(tangentWS, bitangentWS, normalWS));
                ...
                inputData.shadowMask      = SAMPLE_SHADOWMASK(IN.staticLightmapUV);
                    ...
                    inputData.bakedGI = SAMPLE_GI(IN.staticLightmapUV, IN.dynamicLightmapUV, IN.vertexSH, finalNormalWS);
                #else
